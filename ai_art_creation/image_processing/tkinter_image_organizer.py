...
source_dir = "C:\\Users\\trent\\OneDrive\\Documents\\GitHub\\ai_art_creation\\ai_art_creation\\image_processing\\images_raw\\"
ready_to_scale_dir = "C:\\Users\\trent\\OneDrive\\Documents\\GitHub\\ai_art_creation\\ai_art_creation\\image_processing\\images_raw\\ready_to_scale\\"
selected_dir = "C:\\Users\\trent\\OneDrive\\Documents\\GitHub\\ai_art_creation\\ai_art_creation\\image_processing\\images_raw\\selected\\"
removed_dir = "C:\\Users\\trent\\OneDrive\\Documents\\GitHub\\ai_art_creation\\ai_art_creation\\image_processing\\images_raw\\removed\\"
...
def process_image():
    ...
    if var_standard.get():
        shutil.copy2(selected_path, os.path.join(ready_to_scale_dir, f"{base_filename}-rounded{ext}"))
    if var_shapes.get():
        for shape in ["-circle", "-diamond", "-star", "-triangle", "-randpoly", "-randpolycircle"]:
            shutil.copy2(selected_path, os.path.join(ready_to_scale_dir, f"{base_filename}{shape}{ext}"))
    if var_font.get():
        shutil.copy2(selected_path, os.path.join(ready_to_scale_dir, f"{base_filename}-font{ext}"))

    if var_standard.get() or var_shapes.get() or var_font.get():
        shutil.copy2(selected_path, os.path.join(selected_dir, f"{base_filename}{ext}"))
        shutil.move(selected_path, os.path.join(ready_to_scale_dir, selected_file))
