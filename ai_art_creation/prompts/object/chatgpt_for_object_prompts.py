...
from ai_art_creation.prompts.prompt_path_info import object_pickle_file_path, sqlite_db_path, starting_keywords_path, object_products_path, base_prompt_output_path, keywords_folder_path
...
def get_keywords_list(starting_folder_path, 
    ...
    # Load products
    with open(object_products_path, 'r') as f:
        # Read lines into a list
...
def generate_object_prompts(keyword, 
    ...
    sunglasses_coin_toss = random.choice(["with aviator sunglasses", ""])

    system_prompt = f'''As a prompt generator for a generative AI called "Midjourney", you will create image prompts for the AI to visualize on a white #000000 background. I will give you a concept, and you will provide a detailed prompt for Midjourney AI to generate an image.

                        Please adhere to the structure and formatting below, and follow these guidelines:

                        - Specify to put the focus on the object, not the background.
                        - Specify there is to be only one object in the scene, surrounded by empty white #000000 space.
                        - Specify the backgrounnd to surrounded by a white #000000 background every time.
                        - Do not use the words "description" or ":" in any form.
                        - Write each prompt in one line without using return.


                        Structure:
                        [1] = patriotic usa american {keyword} {sunglasses_coin_toss}
                        [2] = a detailed description of [1] with specific imagery details.
                        [3] = a detailed description of the scene's environment.
                        [4] = a detailed description of the scene's mood, feelings, and atmosphere.
                        [5] = A style (e.g. vectorized cartoon, cartoon, geometric, illustration, artwork, 3D, etc.) for [1].
                        [6] = A description of how [5] will be executed (e.g. pixar style, disney style, rendering engine settings, etc.)
                        ...
                        Example Prompts:
                        Prompt 1:
                        /imagine prompt: A galaxy-themed design featuring a cute kitten wearing a space helmet in front of an american flag in a vectorized cartoon style surrounded by a white background.

                        Prompt 2:
                        /imagine prompt: Electric blue cat playing a guitar on a tropical beach at sunset with an american flag in the background in a pixar 3d style surrounded by a white background.
                        '''
                                
    # Define a list of styles
    #style = f'''Along with always specifying "on a white #000000 background, Incorporate a few of the following key phrases in your prompt:
    #            "Geometric",
    #            "Cute",
    #            "Trendy",
    #            "Cool",
    ...
    style = f'''Along with always specifying surrounded by a white #000000 background, Incorporate a few of the following key phrases in your prompt:
                - gaudy
                - flashy
                - vibrant
                - colorful
    ...
    prompts_list = [s + " Surrounded by a white #000000 background" for s in prompts_list]
...
def get_prompts(keywords_list, 
    ...
    output_path = f"{base_prompt_output_path}object\\all_prompts_object_{timestamp}.txt"
...
num_runs = 50 # you can change this to the number of runs you need
...
for i in range(num_runs):
    ...
    while len(completed_keywords_check) < len(keywords_to_run):
        ...
        completed_keywords_check = get_prompts(keywords_to_run, 
                                                     ...
                                                     pickle_file_path=object_pickle_file_path,
        ...
        if len(completed_keywords_check) == len(keywords_to_run):
            ...
            os.remove(object_pickle_file_path)
